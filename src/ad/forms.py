...
class SaveBook(forms.ModelForm):
    ...
    #     try:
    #         models.Book.objects.get(title=title)
    #         self.add_error('title',forms.ValidationError('This book title already exists.'))
    #         self.errors['title'].pop(0)
    #     except:
    #         return {'title':title, 'publication_year':publication_year, 'author':author, 'category':category, 'description':description, 'sourcetype':sourcetype, 'language':language, 'image':image, 'quantity':quantity}
